...
// GET /users/search - Quick user search for meeting participant picker
router.get('/search', authenticate, async (req, res) => {
  try {
    ...
    let whereClause = 'WHERE (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR computer_number LIKE ?)';
    ...
    const [users] = await promisePool.query(
      `SELECT computer_number, first_name, last_name, email, role
       FROM users
       ${whereClause}
    ...
    return res.json({
      success: true,
      data: users.map((u) => ({
        computer_number: u.computer_number,
